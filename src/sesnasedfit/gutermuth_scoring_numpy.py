...
def sigmoid(x: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    ...
    # Safer sigmoid implementation that clips extreme values
    # to prevent overflow without changing global state
...
def compute_pah_scores_batch(fnu_df: pd.DataFrame,
    ...
    # -------------------------------------------------------------------------
    distance_1_2 = -(color_45_58 - 1.05) / scale
    score_1_2 = sigmoid(distance_1_2)
    
    # DIAGNOSTIC for criterion 1.2
    if debug_idx is not None:
        print(f"\nDIAGNOSTIC - Criterion 1.2 at index {debug_idx}:")
        print(f"  color_45_58[{debug_idx}] = {color_45_58.iloc[debug_idx]:.6f}")
        print(f"  distance input = -(color - 1.05) / {scale}")
        print(f"  distance[{debug_idx}] = {distance_1_2.iloc[debug_idx] if hasattr(distance_1_2, 'iloc') else distance_1_2[debug_idx]:.6f}")
        print(f"  score_1_2[{debug_idx}] = {score_1_2[debug_idx]:.6f}")
