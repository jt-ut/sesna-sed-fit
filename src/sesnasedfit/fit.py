...
def fit_and_save_batch(path_fitparm, startindex, endindex, 
    ...
    """
    # Validate inputs
    if not os.path.exists(path_fitparm):
        raise FileNotFoundError(f"Parameter file not found: {path_fitparm}")
    if startindex < 0:
        raise ValueError(f"startindex must be non-negative, got {startindex}")
    if endindex < startindex:
        raise ValueError(f"endindex ({endindex}) must be >= startindex ({startindex})")
    if n_workers <= 0:
...
def generate_batch_script(path_fitparm, batch_size=10000, n_workers=30, 
    ...
    python_executable : str, optional
        Python executable to use in the script (e.g., 'python', 'python3',
        'python3.9', '/path/to/venv/bin/python'). Default: 'python'.
